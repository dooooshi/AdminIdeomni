...
/**
 * The IdeomniRouteItemType type
 */
export type IdeomniRouteItemType = {
	path?: string;
	element?: React.ReactNode;
	auth?: string[] | [];
	settings?: DeepPartial<IdeomniSettingsConfigType>;
	children?: IdeomniRouteItemType[];
};

/**
 * The IdeomniRoutesType type is a custom type that is an array of IdeomniRouteItemType objects.
 */
export type IdeomniRoutesType = IdeomniRouteItemType[];

/**
 * The IdeomniRouteConfigType type is a custom type that defines the configuration for a set of routes.
 * It includes an optional routes property, an optional settings property, and an optional auth property.
 */
export type IdeomniRouteConfigType = {
	routes: IdeomniRoutesType;
	settings?: PartialDeep<IdeomniSettingsConfigType>;
	auth?: string[] | [];
};

/**
 * The IdeomniRouteConfigsType type is a custom type that is an array of IdeomniRouteConfigType objects.
 */
export type IdeomniRouteConfigsType = IdeomniRouteConfigType[] | [];
...
/**
 * The IdeomniUtils class provides utility functions for the Ideomni project.
 */
class IdeomniUtils {
	/**
	...
	/**
	 * The setRoutes function sets the routes for the Ideomni project.
	 */
	static setRoutes(
		config: IdeomniRouteConfigType,
		defaultAuth: IdeomniSettingsConfigType['defaultAuth'] = undefined
	): IdeomniRouteItemType[] {
		let routes: IdeomniRouteItemType[] = [];

		if (config?.routes) {
			routes = [...config.routes];
		}

		const applyAuth = (route: IdeomniRouteItemType, parentAuth: string[] | null) => {
	...
	static generateRoutesFromConfigs(
		configs: IdeomniRouteConfigsType,
		defaultAuth: IdeomniSettingsConfigType['defaultAuth']
	) {
		let allRoutes: IdeomniRouteItemType[] = [];
		configs.forEach((config: IdeomniRouteConfigType) => {
