...
/**
 * The IdeomniShortcuts component is responsible for rendering a list of shortcuts based on the navigation and shortcuts props.
...
function IdeomniShortcuts(props: IdeomniShortcutsProps) {
	...
	const [searchResults, setSearchResults] = useState<IdeomniFlatNavItemType[]>([]);
	const [shortcutItems, setShortcutItems] = useState<IdeomniFlatNavItemType[]>([]);

	useEffect(() => {
		const _shortcutItems = shortcuts
			? shortcuts.map((id) => _.find(navigation, { id }))
			: ([] as IdeomniFlatNavItemType[]);
									...
									>
										...
										>
											<IconButton className="h-9 w-9 p-0 rounded-none">
												{_item.icon ? (
													<IdeomniSvgIcon size={20}>{_item.icon}</IdeomniSvgIcon>
												) : (
...
function ShortcutMenuItem(props: {
	shortcuts: IdeomniShortcutsProps['shortcuts'];
	item: IdeomniFlatNavItemType;
		...
		>
			<MenuItem key={item.id}>
				<ListItemIcon className="min-w-9">
					{item.icon ? (
						<IdeomniSvgIcon>{item.icon}</IdeomniSvgIcon>
					) : (
...
export default memo(IdeomniShortcuts);
