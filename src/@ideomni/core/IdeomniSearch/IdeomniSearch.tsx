...
import useNavigate from '@ideomni/hooks/useNavigate';
import IdeomniSvgIcon from '../IdeomniSvgIcon';
import { IdeomniFlatNavItemType } from '../IdeomniNavigation/types/IdeomniNavItemType';

const Root = styled('div')(({ theme }) => ({
	'& .IdeomniSearch-container': {
		position: 'relative'
	},
	'& .IdeomniSearch-suggestionsContainerOpen': {
		position: 'absolute',
		zIndex: 1,
	...
	'& .IdeomniSearch-suggestion': {
		display: 'block'
	},
	'& .IdeomniSearch-suggestionsList': {
		margin: 0,
		padding: 0,
		listStyleType: 'none'
	},
		...
		>
			{variant === 'basic' ? (
				...
				<>
					<TextField
						...
						slotProps={{
							input: {
								...
								classes: {
									input: 'IdeomniSearch-input py-0 px-4 h-9 md:h-9 pr-9',
					...
					/>
					<IdeomniSvgIcon
						className="pointer-events-none absolute top-0 h-9 w-9 p-2 right-0"
						color="action"
						size={20}
					>
				...
				<TextField
					...
					slotProps={{
						input: {
							...
							classes: {
								input: 'IdeomniSearch-input py-0 px-4 h-9'
							}
						}
					}}
					variant="standard"
...
function renderSuggestion(suggestion: IdeomniFlatNavItemType, { query, isHighlighted }: RenderSuggestionParams) {
	...
	return (
		...
		>
			<ListItemIcon className="min-w-9">
				{suggestion.icon ? (
					<IdeomniSvgIcon>{suggestion.icon}</IdeomniSvgIcon>
				) : (
...
function getSuggestions(value: string, data: IdeomniFlatNavItemType[]): IdeomniFlatNavItemType[] {
...
function getSuggestionValue(suggestion: IdeomniFlatNavItemType) {
...
type StateType = {
	searchText: string;
	search: boolean;
	navigation: IdeomniFlatNavItemType[];
	suggestions: IdeomniFlatNavItemType[];
...
type ActionType =
	...
	| { type: 'setNavigation'; data: IdeomniFlatNavItemType[] }
...
/**
 * Props for IdeomniSearch component
 */
type IdeomniSearchProps = {
	className?: string;
	navigation: IdeomniFlatNavItemType[];
	variant?: 'basic' | 'full';
	trigger?: ReactNode;
...
/**
 * IdeomniSearch component
 */
function IdeomniSearch(props: IdeomniSearchProps) {
	const {
		navigation = [],
		className,
		variant = 'full',
		...
		trigger = (
			<IconButton className="border border-divider">
				<IdeomniSvgIcon size={20}>heroicons-outline:magnifying-glass</IdeomniSvgIcon>
			</IconButton>
		)
	} = props;
	...
	function handleSuggestionSelected(
		...
		{ suggestion }: { suggestion: IdeomniFlatNavItemType }
	) {
				...
				>
					<Autosuggest
						...
						theme={{
							...
							suggestionsList: 'IdeomniSearch-suggestionsList',
							suggestion: 'IdeomniSearch-suggestion'
						}}
...
export default memo(IdeomniSearch);
