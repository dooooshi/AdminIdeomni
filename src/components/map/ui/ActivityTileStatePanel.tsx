...
const ActivityTileStatePanel: React.FC<ActivityTileStatePanelProps> = ({
  ...
  useEffect(() => {
    if (selectedTileState) {
      setCurrentPrice(selectedTileState.newPrice || selectedTileState.tile.initialPrice || 0);
  ...
  const handleResetToDefaults = () => {
    if (selectedTileState) {
      setCurrentPrice((selectedTileState.tile.initialPrice || 0));
  ...
  const calculateMetrics = () => {
    ...
    const originalValue = AdminTileStateService.calculateTileValue(
      (selectedTileState.tile.initialPrice || 0),
      selectedTileState.tile.initialPopulation
    );
    
    const priceChange = currentPrice - (selectedTileState.tile.initialPrice || 0);
    ...
    return {
      ...
      populationChange,
      valueChange,
      priceChangePercent: (priceChange / (selectedTileState.tile.initialPrice || 0)) * 100,
