...
function GoldTransferPage() {
  ...
  const [transferGold, { isLoading: isTransferring }] = useTransferGoldMutation();
  ...
  useEffect(() => {
    ...
    const request: TransferGoldRequest = {
  ...
  const confirmationData = formState.selectedTeamId && formState.amount ? {
    ...
    amount: parseFloat(formState.amount),
    currentBalance: teamAccount.gold,
    balanceAfter: teamAccount.gold - parseFloat(formState.amount)
  } : null;
